...
## 15. Three-week scope and work plan

The implementation is designed as a narrow, reproducible three-week course prototype.

| Week | Tasks | Output |
|---|---|---|
| Week 1 | Select CASAS Aruba, define input/output, parse CSV/TXT events, normalize timestamps, create daily routine features and document the dataset protocol. | Clean event pipeline, `features_daily.csv`, dataset protocol. |
| Week 2 | Implement rule-based anomaly detection, Isolation Forest, privacy levels P0-P3, synthetic anomaly injection and evaluation metrics. | `predictions_combined.csv`, `metrics.json`, false-positive/false-negative analysis. |
| Week 3 | Build Streamlit dashboard, finalize caregiver notification policy, write ethical review, README and IEEE-style documentation. | Dashboard, final PDF, reproducible commands and screenshots. |

---

## 16. Initial assignment coverage checklist

| Requirement from project brief | Covered by this project |
|---|---|
| Ambient Assisted Living and Routine Monitoring | Uses ambient smart-home sensor events to model one resident's daily routine. |
| Daily routine anomaly detection | Detects inactivity, unusual nighttime movement, missed morning routine and missing-data/sensor-failure cases. |
| Ambient sensors | Uses CASAS Aruba ambient sensor events such as room activity and ON/OFF events. |
| Time-series modeling | Converts event streams into daily and period-based routine features. |
| Anomaly detection | Implements a rule-based baseline and an Isolation Forest model. |
| Privacy-by-design | Uses minimization, aggregation and pseudonymization levels P0-P3. |
| Dashboard | Includes a Streamlit dashboard in `dashboard/app.py`. |
| Data minimization | Dashboard and model use summarized features, not permanent raw event display. |
| False-positive analysis | Evaluation reports TP, TN, FP, FN, precision, recall and F1. |
| Caregiver notification policy | Levels 0-3 are implemented in `notification_policy.py`. |
| Ethical review | The report states no cameras, no microphones, no third-party surveillance and no clinical claims. |
| Negative/failure-case experiment | Synthetic sensor failure/missing-data case is included. |
| Prototype | Provided as runnable Python code. |
| Dataset protocol | Included in `reports/dataset_protocol.md` and the PDF. |
| Evaluation report | Supported by `outputs/metrics.json`, `outputs/predictions_combined.csv` and the report sections. |
| IEEE-style paper | Template is included in `reports/ieee_paper_template.md`; the PDF follows a structured report format. |

Note: the assignment paragraph about Tor/VPN/direct connection appears to be copied from a different anonymity project. In this implementation, that baseline is replaced with a smart-home-relevant privacy comparison: raw-derived daily features vs. pseudonymized, aggregated and minimized representations.
