...
def _is_motion_event(df: pd.DataFrame) -> pd.Series:
    ...
    sensor_type = (
        df.get("sensor_type", pd.Series("other", index=df.index))
        .astype(str)
        .str.lower()
    )
...
def add_sensor_metadata(
    events: pd.DataFrame, sensor_groups: Dict[str, List[str]] | None = None
) -> pd.DataFrame:
    events = events.copy()
...
def longest_gap_minutes(timestamps: pd.Series) -> float:
    ...
    all_points = pd.concat(
        [pd.Series([day_start]), ts, pd.Series([day_end])]
    ).sort_values()
...
def extract_daily_features(
    ...
    for date in all_dates:
        ...
        night = day_motion[
            (day_motion["hour"] >= tw.get("night_start_hour", 0))
            & (day_motion["hour"] <= tw.get("night_end_hour", 5))
        ]
        morning = day_motion[
            (day_motion["hour"] >= tw.get("morning_start_hour", 6))
            & (day_motion["hour"] <= tw.get("morning_end_hour", 11))
        ]
        daytime = day_motion[
            (day_motion["hour"] >= tw.get("day_start_hour", 12))
            & (day_motion["hour"] <= tw.get("day_end_hour", 17))
        ]
        evening = day_motion[
            (day_motion["hour"] >= tw.get("evening_start_hour", 18))
            & (day_motion["hour"] <= tw.get("evening_end_hour", 23))
        ]

        first_motion = (
            float(day_motion["minute_of_day"].min()) if not day_motion.empty else np.nan
        )
        last_motion = (
            float(day_motion["minute_of_day"].max()) if not day_motion.empty else np.nan
        )
        ...
        row = {
            ...
            "longest_inactivity_gap_minutes": longest_gap_minutes(
                day_motion["timestamp"]
            ),
    ...
    features[numeric_cols] = (
        features[numeric_cols]
        .fillna(features[numeric_cols].median(numeric_only=True))
        .fillna(0)
    )
    return features
...
def extract_hourly_features(
    ...
    grouped = (
        events.groupby(["date", "hour", "sensor_type"]).size().reset_index(name="count")
    )
    pivot = grouped.pivot_table(
        index="date", columns=["sensor_type", "hour"], values="count", fill_value=0
    )
    pivot.columns = [
        f"{sensor_type}_h{hour:02d}" for sensor_type, hour in pivot.columns
    ]
